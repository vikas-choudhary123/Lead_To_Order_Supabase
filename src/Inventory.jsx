...
const handleSaveStockInline = async (product) => {
  ...
  try {
    ...
    // We'll log stock movements but won't calculate new stock ourselves
    // Instead, we'll rely on the sheet's value to be updated externally
    
    // Just log the values for reference
    console.log(`Current stock: ${currentStock}, Stock In: ${stockInValue}, Stock Out: ${stockOutValue}`);
    
    // Check if trying to remove more than available (just for validation)
    ...
    // We no longer update the main inventory sheet's stock count
    // This will be handled by the sheet formulas or external processes
    
    console.log("Stock movement logged successfully - the sheet will handle stock calculations");
    
    // We're no longer automatically updating the stock status
    // This will be handled by the sheet formulas or external processes
    
    console.log("Stock status will be updated by the sheet or external process");
    
    // We won't update our local product state with calculated values
    // Instead, we'll wait for the next data refresh to get the updated values from the sheet
    
    console.log("Local state will be updated on next data fetch");
    
    // Note: If you want to implement an immediate data refresh after saving, 
    // you could call fetchGoogleSheetData() here to get fresh data from the sheet
