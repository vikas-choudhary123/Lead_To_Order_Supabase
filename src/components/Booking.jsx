...
const Booking = ({ hideHistoryButton = false }) => {
  ...
  const handleNewAppointmentClick = async () => {
    ...
    const bookingIdHeader = tableHeaders.find(header => 
      ...
      (header.label.toLowerCase().includes('id') && !header.label.toLowerCase().includes('serial'))
    );
